...
import type { EventCard } from "@/server/queries/events";
    ...
    <div className="flex w-full flex-col items-center gap-[30px] md:gap-10 lg:gap-[50px]">
      <div role="tablist" aria-label="Filter by date" className="flex flex-wrap items-center justify-center gap-[10px]">
        {TABS.map((name) => (
          <button key={name} type="button" role="tab" aria-selected={tab === name} onClick={() => setTab(name)} className="relative h-[38px] overflow-clip rounded-full bg-surface px-5 text-[14px] font-medium leading-[18.2px]">
            {tab === name && <motion.span layoutId="events-tab" className="absolute inset-0 bg-[#100F12]" transition={{ type: "spring", bounce: 0, duration: 0.5 }} />}
      ...
      {shown.length === 0 ? (
        ...
        <motion.div layout className="grid w-full gap-5 md:grid-cols-2 md:gap-[30px] lg:grid-cols-3">
          {shown.map((event, i) => (
            <motion.div key={event.article.slug} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", bounce: 0, duration: 0.6, delay: Math.min(i * 0.04, 0.4) }} className="flex flex-col gap-[10px]">
