...
import type { FilterOption } from "@/server/queries/catalogue";
...
export function CourseFilters({ query, options }: { query: CourseQuery; options: FilterOptions }) {
  ...
  return (
    <div className="flex w-full flex-col gap-5 md:gap-[30px]">
      <form method="get" action="/courses" className="grid w-full gap-5 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Field label="Keyword" name="q" placeholder="Course, institution or country" />
        <Select label="Destination" name="destination" defaultValue={query.destination ?? ""}>
          <option value="">All destinations</option>
          {options.destinations.map((d) => <option key={d.slug} value={d.slug}>{d.name}</option>)}
        </Select>
        <Select label="Qualification level" name="level" defaultValue={query.level ?? ""}>
          <option value="">All levels</option>
          {LEVELS.map((l) => <option key={l.value} value={l.value}>{l.label}</option>)}
        </Select>
        <Select label="Category" name="category" defaultValue={query.category ?? ""}>
          <option value="">All categories</option>
          {options.categories.map((c) => <option key={c.slug} value={c.slug}>{c.name}</option>)}
        </Select>
        <Select label="Institution" name="institution" defaultValue={query.institution ?? ""}>
          <option value="">All institutions</option>
          {options.institutions.map((i) => <option key={i.slug} value={i.slug}>{i.name}</option>)}
        </Select>
        <Select label="Intake" name="intake" defaultValue={query.intake ?? ""}>
      ...
      {active.length > 0 && (
        <div className="flex flex-wrap items-center gap-[10px]">
          {active.map((f) => (
            <Link key={f.key} href={filterHref(clearFilter(query, f.key))} aria-label={`Remove filter ${names.get(f.value) ?? f.value}`}>
              <Chip>{names.get(f.value) ?? f.value} ✕</Chip>
            </Link>
          ))}
          <Link href="/courses"><Chip tone="white">Clear all</Chip></Link>
        </div>
      )}
    </div>
  );
